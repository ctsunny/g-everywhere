#!/bin/bash
# G-Everywhere v3.9 (WG 启动彻底修复版)
# https://github.com/ctsunny/g-everywhere
# 修复版 by Grok (xAI) - 2026.3
# 本次修复重点：
#   1. 剥离 IPv6 Address（系统已禁用 IPv6，原配置带 IPv6 会导致 wg-quick 失败）
#   2. AllowedIPs 强制 IPv4 全通（更干净）
#   3. start_wg 显示真实 wg-quick 错误（不再静默失败）
#   4. 启动逻辑优化 + 模块检查
...
show_banner() {
    ...
    echo -e " ${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e " ${BLUE}github.com/ctsunny/g-everywhere${NC} │ ${GREEN}v3.9 (WG启动已修复)${NC}\n"
}
...
setup_config() {
    ...
    # ★★★ v3.9 关键修复：剥离 IPv6（系统已禁用 IPv6，否则 wg-quick 会报 RTNETLINK 错误）
    sed -i '/^Address = /s/, .*\/128//' /tmp/wgcf-profile.conf
    sed -i '/^AllowedIPs = /c\AllowedIPs = 0.0.0.0/0' /tmp/wgcf-profile.conf
    echo -e " ${GREEN}✓ 已剥离 IPv6（仅保留 IPv4 全通路由）${NC}"
...
start_wg() {
    ...
    echo -e "${YELLOW}正在启动 WireGuard 接口...（错误将直接显示）${NC}"
    if wg-quick up ${WG_CONF} 2>&1; then
        sleep 2
        echo -e " ${GREEN}✓ WireGuard 接口启动成功${NC}"
    else
        echo -e " ${RED}WireGuard 启动失败！上方为详细错误${NC}"
        echo -e " ${YELLOW}建议运行: journalctl -xe | grep -E 'wg|wireguard'${NC}"
        exit 1
    fi
...
[Unit]
Description=G-Everywhere WireGuard Google Routing (v3.9 Fixed)
...
do_install() {
    ...
    echo -e "\n${BOLD}${GREEN}"
    echo " ┌──────────────────────────────────────────┐"
    echo " │ ✅ 安装成功！Google 已解锁 (v3.9 Fixed) │"
    echo " └──────────────────────────────────────────┘"
    echo -e "${NC}"
