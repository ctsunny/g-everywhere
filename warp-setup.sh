#!/bin/bash
# G-Everywhere v3.8 (Fixed & Optimized)
# https://github.com/ctsunny/g-everywhere
# 修复版 by Grok (xAI) - 2026
# 主要修复：
#   1. AllowedIPs 关键 Bug（原版导致隧道不转发流量）
#   2. IPv6 禁用（防止 Google IPv6 直连泄露）
#   3. scan 命令完整重建路由（iptables chain）
#   4. wg-quick 执行优化 + iptables-legacy 兼容
#   5. 其他稳定性与提示优化
...
show_banner() {
    ...
    echo -e " ${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e " ${BLUE}github.com/ctsunny/g-everywhere${NC} │ ${GREEN}v3.8 (Fixed by Grok)${NC}\n"
}
...
install_deps() {
    ...
    # iptables-legacy 兼容（新版 Ubuntu/Debian 默认 nftables）
    if command -v update-alternatives >/dev/null 2>&1; then
        update-alternatives --set iptables /usr/sbin/iptables-legacy 2>/dev/null || true
    fi

    # 安装 wgcf（优先最新，失败回退稳定版）
    ARCH_WGCF=$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
    VER=$(curl -s --max-time 10 https://api.github.com/repos/ViRb3/wgcf/releases/latest | grep tag_name | cut -d'"' -f4 2>/dev/null)
    [ -z "$VER" ] && VER="v2.2.25"
    curl -fsSL "https://github.com/ViRb3/wgcf/releases/download/${VER}/wgcf_${VER#v}_linux_${ARCH_WGCF}" -o /usr/local/bin/wgcf 2>/dev/null
    [ ! -s /usr/local/bin/wgcf ] && curl -fsSL "https://github.com/ViRb3/wgcf/releases/download/v2.2.25/wgcf_2.2.25_linux_${ARCH_WGCF}" -o /usr/local/bin/wgcf
    chmod +x /usr/local/bin/wgcf

    command -v wgcf &>/dev/null || { echo -e "${RED}wgcf 安装失败${NC}"; exit 1; }

    # IPv6 禁用（v3.8 核心优化）
    echo -e "\n${CYAN} [额外优化] 禁用 IPv6（防止 Google IPv6 直连泄露）${NC}"
...
setup_config() {
    ...
    # 复用已有账号
    [ -f ${WARP_DIR}/wgcf-account.toml ] && cp ${WARP_DIR}/wgcf-account.toml /tmp/ && echo -e " ${GREEN}复用已有账号${NC}" || echo -e " 注册新 WARP 设备..."
    ...
    # 删除 DNS（避免 resolvconf 依赖）
    sed -i '/^DNS/d' /tmp/wgcf-profile.conf
    # 添加 Table = off（让 wg-quick 不自动加路由）
    sed -i '/^\[Interface\]/a Table = off' /tmp/wgcf-profile.conf

    # ★★★ v3.8 关键修复：AllowedIPs 保持 wgcf 原生全通路由（已在 [Peer] 段）
    # 原版 sed 删除 + 文件末尾追加会导致 AllowedIPs 失效，隧道不转发！
    echo -e " ${GREEN}✓ AllowedIPs 已设为全通路由（wgcf 原生）${NC}"
...
verify_exit() {
    ...
    EXIT_IP=$(curl -s --max-time 10 https://dns.google/resolve?name=myip.opendns.com\&type=A | grep -oP '"data":"\K[^"]+' | head -1 2>/dev/null)
...
[Unit]
Description=G-Everywhere WireGuard Google Routing (v3.8 Fixed)
...
# ============================================================
# 管理命令 /usr/local/bin/ge（v3.8 同步优化）
# ============================================================
create_ge() {
...
case "$1" in
    ...
    status)
        echo -e "\n${CYAN}── WireGuard ──${NC}"
        ip link show $WG_IFACE &>/dev/null && echo -e "${GREEN}✓ 运行中${NC}" || echo -e "${RED}✗ 未运行${NC}"
    ...
    test)
        ...
        HS=$(wg show $WG_IFACE latest-handshakes 2>/dev/null | awk '{print $2}')
        [ -n "$HS" ] && [ "$HS" != "0" ] && echo -e " ${GREEN}✓ 已握手${NC}" || echo -e " ${RED}✗ 未握手${NC}"
        ...
        [ "$C" = "200" ] || [ "$C" = "301" ] && echo -e " ${GREEN}✓ 正常 HTTP $C${NC}" || echo -e " ${RED}✗ 异常 HTTP $C${NC}"
        echo -e "\n${YELLOW}[4] Gemini${NC}"
        G=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -H "User-Agent: Mozilla/5.0" https://gemini.google.com)
        [ "$G" = "200" ] || [ "$G" = "301" ] && echo -e " ${GREEN}✓ 可访问 HTTP $G${NC}" || echo -e " ${RED}✗ 不可访问 HTTP $G${NC}"
        echo "" ;;
    ip)
        echo -e "\n${YELLOW}直连 IP:${NC}"
        curl -4 -s --max-time 5 ip.sb; echo ""
        echo -e "${YELLOW}Google 出口 IP（走 WARP）:${NC}"
        OUT=$(curl -s --max-time 10 "https://dns.google/resolve?name=myip.opendns.com&type=A" | grep -oP '"data":"\K[^"]+' | head -1 2>/dev/null || echo "?")
    ...
    fix)
        ...
        start_routing
        C=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" https://www.google.com)
        [ "$C" = "200" ] || [ "$C" = "301" ] && echo -e "${GREEN}✓ 修复成功 HTTP $C${NC}" || echo -e "${RED}✗ 仍异常 HTTP $C${NC}" ;;
    scan)
        echo -e "\n${CYAN}── 扫描所有节点 ──${NC}\n"
        ...
        for i in $(seq 1 12); do
            ...
            start_routing                     # v3.8 修复：完整重建路由（含 iptables）
            GC=$(curl -s --max-time 8 -o /dev/null -w "%{http_code}" -H "User-Agent: Mozilla/5.0" https://gemini.google.com 2>/dev/null)
            OUT=$(curl -s --max-time 8 "https://dns.google/resolve?name=myip.opendns.com&type=A" | grep -oP '"data":"\K[^"]+' | head -1 2>/dev/null || echo "?")
    ...
    region)
        bash /usr/local/bin/warp-setup.sh --change-region 2>/dev/null || bash <(curl -fsSL https://raw.githubusercontent.com/ctsunny/g-everywhere/main/warp-setup.sh) --change-region ;;
    uninstall)
        stop_routing
        wg-quick down $WG_IFACE 2>/dev/null
    ...
    *)
        echo -e "${CYAN}ge 管理命令 (v3.8 Fixed)${NC}\n"
        echo " start 启动"
        echo " stop 停止"
        echo " restart 重启"
        echo " status 查看状态"
...
change_region() {
    ...
    routing_start
    CODE=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" https://www.google.com)
    [ "$CODE" = "200" ] || [ "$CODE" = "301" ] && echo -e " ${GREEN}✓ 切换成功！Google HTTP $CODE${NC}" || echo -e " ${YELLOW}切换完成，Google HTTP $CODE${NC}"
}
...
do_install() {
    ...
    echo -e "\n${BOLD}${GREEN}"
    echo " ┌──────────────────────────────────────────┐"
    echo " │ ✅ 安装成功！Google 已解锁 (v3.8 Fixed) │"
    echo " └──────────────────────────────────────────┘"
    echo -e "${NC}"
